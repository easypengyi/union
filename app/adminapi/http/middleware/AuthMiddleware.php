...
class AuthMiddleware
{
    ...
    public function handle($request, \Closure $next)
    {
        ...
        //文件上传无需权限 2022-03-17
        $controllerArr = ['file'];
        if(in_array(strtolower($request->controller()), $controllerArr)){
            return $next($request);
        }
        //管理员访问的uri判断
        $AdminUris = $adminAuthCache->getAdminUri()['action_auth'] ?? [];
//        file_put_contents('../runtime/admin_auth.' . date('Ymd') . '.log', date('Y-m-d H:i:s') . ' STEP1:当前权限 ' . $accessUri . "\n", FILE_APPEND);
//        file_put_contents('../runtime/admin_auth.' . date('Ymd') . '.log', date('Y-m-d H:i:s') . ' STEP2:总权限 ' . json_encode($AdminUris) . "\n", FILE_APPEND);
