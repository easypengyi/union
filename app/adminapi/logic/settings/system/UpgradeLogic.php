...
class UpgradeLogic extends BaseLogic
{
     ...
     */
    public static function formatLists($lists)
    {
    ...
    public static function upgrade($params)
    {
        ...
        if(strpos($openBasedir, "server") !== false) {
            self::$error = '请参考技术社区对应产品升级文章配置open_basedir';
            return false;
        }
        ...
        try {
            ...
            //更新文件
            if (false === self::upgradeFile($tempDir . 'project/', self::getProjectPath())) {
    ...
    {
        $cacheVersion = Cache::get('version_lists');
        if (!empty($cacheVersion)) {
            return $cacheVersion;
        }
        if (empty($pageNo) || empty($pageSize)) {
            $remoteUrl = self::BASE_URL . "/api/version/lists?product_id=3&type=2";
        } else {
            $remoteUrl = self::BASE_URL . "/api/version/lists?product_id=3&type=2&page_no=$pageNo&page_size=$pageSize";
        }
        $result = Requests::get($remoteUrl);
        $result = json_decode($result->body, true);
        $result = $result['data'] ?? [];
    ...
    {
        $cacheVersion = self::getRemoteVersion();
        if (!empty($cacheVersion)) {
