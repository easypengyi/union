 ...
 */
class GroupLevelLogic extends BaseLogic
{
    /**
     * @notes 添加分销会员等级
     * @param $params
    ...
    public static function add($params)
    {
        ...
        try{
            ...
            if(!empty($params['id'])){
                $newLevel =  GroupLevel::update($params);
            }else{
                $newLevel = GroupLevel::create($params);
            }
    ...
    public static function delete($params)
    {
        ...
        Db::startTrans();
        try{
            $level = GroupLevel::findOrEmpty($params['id']);
            if($level->isEmpty()) {
                throw new \Exception('等级不存在');
            }

            //是否已关联用户
            $count = GroupOperator::where('level_id', $level['id'])->count();
            if($count > 0){
                throw new \Exception('已关联，请先删除等级下的运营商');
            }
