...
class GoodsActivityValidate extends BaseValidate
{
    protected $rule = [
        'id' => 'require|checkId',
    ...
    {
        return $this->only(['id','name','pid','sort','is_recommend'])
            ->append('pid','checkPid');
    }
    ...
    {
        $result = GoodsActivity::findOrEmpty($value);

        $is_update = GoodsActivityLog::where('activity_id', $value)->find();

        if ($result->isEmpty() && is_null($is_update)) {
            return false;
            //return '活动不存在';
        }
