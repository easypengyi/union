...
use app\job\UpdateActivityGoods;
...
class AkcActivityGoodsUp extends Command
{

    protected function configure()
    {
        $this->setName('update_activity_goods')
            ->setDescription('更新爱库存专场活动商品');
    }
    ...
    protected function execute(Input $input, Output $output)
    {
        $__startTime = time();
        set_time_limit(0);
        Log::info('更新专场活动商品********队列开始');
        //获取期间的商品数量
        $activity_list = (new GoodsActivityLogic())->getActivityGoodsLists();
        if(count($activity_list) == 0){
            Log::info('更新专场活动商品********无商品需要更新');
            return ;
        }
        foreach ($activity_list as $activity_id){
            $queue_data = [
                'activity_id'=> $activity_id
            ];
            Log::info('更新专场活动商品********队列添加.'. json_encode($queue_data));
            $res = Queue::push(UpdateActivityGoods::class, $queue_data, UpdateActivityGoods::NAME);
            Log::info('更新专场活动商品********队列结果.'. json_encode($res));
        }
